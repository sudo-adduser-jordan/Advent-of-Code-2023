...
namespace Solutions05B
{
	...
	void PrintSolution(const std::filesystem::path &inputFile)
	{
		...
		std::vector<ulint> seeds = Utilities::string_to_long_long_vector(line.substr(line.find(':') + 1));
		...
		ulint result = std::ranges::min(std::ranges::views::all(seedRanges) | std::views::transform([](auto &range)
																									{ return range.start; }));
