...
namespace Solutions07B
{
	...
	static Type GetHandType(const std::string &hand)
	{
	...
	static std::vector<Hand> ReadInput(const std::filesystem::path &inputFile)
	{
		// Read hands and bids
		std::vector<Hand> hands;
		for (const std::string &line : Utilities::file_into_string_vector(inputFile))
		{
			std::vector<std::string> split = Utilities::string_split(line, ' ');
